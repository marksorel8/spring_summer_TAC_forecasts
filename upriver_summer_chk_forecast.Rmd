---
title: "TAC spring summer Forecasts"
author: WDFW
output:
  html_document:
    code_folding: hide
...
TAC estimates of river mouth run size from run reconstruction for the folowing stocks:
 - upriver spring Chinook 
 - upper Columbia wild spring Chinook?
 - Sockeye 
 - Summer Chinook
...
Moving Average (ARIMA) models to salmon runsize. The
...
up_sum_chk<-readxl::read_xlsx("data/SummerChinook.xlsx",sheet=1) %>% 
  brood_to_return() %>% 
    mutate(
...
dat<-make_dat(file_path="data/processed/up_sum_chk_2023.csv",dat1=up_sum_chk,redo=FALSE)
...
save(results,file="results/forecasts_up_sum_chk_2023.rds")
}else{
  load("results/forecasts_up_sum_chk_2023.rds")
}
...
rp$all_mods %>% filter(year>=2003) %>% group_by(model) %>% summarize(n=sum(is.na(predicted_abundance))) %>% pull(n) %>% table

num_models<-10
ens<-ensemble(rp$all_mods  %>% group_by(year) %>% mutate(rank=rank(MAPE)) %>% ungroup,dat,TY_ensemble=TY_ensemble,k,slide=15)
# ens$forecast_skill %>%filter(grepl("weight",model))


ens$ensembles %>% filter(grepl("MAPE",model)) %>% left_join(ens$forecasts %>% dplyr::select(year,abundance) %>% distinct) %>% write_csv("summer_chk_arima.csv")
